<div align="center">

<p align="center">
  <img src="pilegal_logo.png" alt="PiLegal Logo" width="240"/>
</p>

# 🤖 PiLegal Data V1

### 🚀 AI Training Dataset Platform

[![Python](https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white)](https://www.python.org/)
[![Data](https://img.shields.io/badge/Dataset-4.1M-FF6B6B?style=for-the-badge&logo=databricks&logoColor=white)](https://github.com/beydeveloper/pilegal)
[![Instagram](https://img.shields.io/badge/Instagram-E4405F?style=for-the-badge&logo=instagram&logoColor=white)](https://www.instagram.com/beydeveloper/)

```ascii
╔═══════════════════════════════════════════════════════════════╗
║                                                               ║
║   ██████╗ ██╗██╗     ███████╗ ██████╗  █████╗ ██╗           ║
║   ██╔══██╗██║██║     ██╔════╝██╔════╝ ██╔══██╗██║           ║
║   ██████╔╝██║██║     █████╗  ██║  ███╗███████║██║           ║
║   ██╔═══╝ ██║██║     ██╔══╝  ██║   ██║██╔══██║██║           ║
║   ██║     ██║███████╗███████╗╚██████╔╝██║  ██║███████╗      ║
║   ╚═╝     ╚═╝╚══════╝╚══════╝ ╚═════╝ ╚═╝  ╚═╝╚══════╝      ║
║                                                               ║
║           🎯 AI Training • 📊 Big Data • 🔍 Query System     ║
║                                                               ║
╚═══════════════════════════════════════════════════════════════╝
```

---

### 🌍 Choose Your Language | Dilinizi Seçin

[🇬🇧 English](#-english) • [🇹🇷 Türkçe](#-türkçe) • [🇸🇦 العربية](#-العربية) • [🇷🇺 Русский](#-русский)

---

</div>

<br>

## 🇬🇧 English

<div align="center">

### 🎯 Purpose

</div>

This data and software are intended for AI developers and for training my own AI model that I am currently developing. Additionally, software developers can gain experience in this large-scale software project.

<div align="center">

### ✨ Features

</div>

> 📊 **4.1 Million** training data available and there is a system that allows you to query within the data. This data will be expanded in the future.

<div align="center">

### 🚀 Installation

</div>

```bash
# Install required libraries
pip install requirements.txt

# Run the server
python server.py
```

<div align="center">

### 📞 Contact

**Instagram:** [beydeveloper](https://www.instagram.com/beydeveloper/)

</div>

---

<br>

## 🇹🇷 Türkçe

<div align="center">

### 🎯 Amaç

</div>

Bu data ve yazılım ai geliştirenler ve kendi geliştirmekte olduğum ai modelinin eğitimi için kullanmayı amaçlıyorum aynı zamanda yazılım geliştiriciler buradaki büyük yazılım projesinde deneyim kazanabilirler

<div align="center">

### ✨ Özellikler

</div>

> 📊 **4.1 Milyon** eğitim datası mevcut ve datalar içinde sorgu yapabilmenizi sağlayan bir sistemde mevcutdur ilerliyen zamanlarda bu data genişletilecektir

<div align="center">

### 🚀 Kurulum

</div>

```bash
# Gerekli kütüphaneleri yükle
pip install requirements.txt

# Sunucuyu çalıştır
python server.py
```

<div align="center">

### 📞 İletişim

**Instagram:** [beydeveloper](https://www.instagram.com/beydeveloper/)

</div>

---

<br>

<div dir="rtl">

## 🇸🇦 العربية

<div align="center">

### 🎯 الهدف

</div>

هذه البيانات والبرمجيات مخصصة لمطوري الذكاء الاصطناعي ولتدريب نموذج الذكاء الاصطناعي الخاص بي الذي أقوم بتطويره حالياً. بالإضافة إلى ذلك، يمكن لمطوري البرمجيات اكتساب الخبرة في هذا المشروع البرمجي الكبير.

<div align="center">

### ✨ الميزات

</div>

> 📊 **4.1 مليون** بيانات تدريبية متاحة ويوجد نظام يتيح لك الاستعلام داخل البيانات. سيتم توسيع هذه البيانات في المستقبل.

<div align="center">

### 🚀 التثبيت

</div>

```bash
# تثبيت المكتبات المطلوبة
pip install requirements.txt

# تشغيل الخادم
python server.py
```

<div align="center">

### 📞 التواصل

**Instagram:** [beydeveloper](https://www.instagram.com/beydeveloper/)

</div>

</div>

---

<br>

## 🇷🇺 Русский

<div align="center">

### 🎯 Цель

</div>

Эти данные и программное обеспечение предназначены для разработчиков искусственного интеллекта и для обучения моей собственной модели ИИ, которую я в настоящее время разрабатываю. Кроме того, разработчики программного обеспечения могут получить опыт в этом крупномасштабном программном проекте.

<div align="center">

### ✨ Особенности

</div>

> 📊 Доступно **4,1 миллиона** обучающих данных, и есть система, которая позволяет делать запросы внутри данных. Эти данные будут расширены в будущем.

<div align="center">

### 🚀 Установка

</div>

```bash
# Установите необходимые библиотеки
pip install requirements.txt

# Запустите сервер
python server.py
```

<div align="center">

### 📞 Контакты

**Instagram:** [beydeveloper](https://www.instagram.com/beydeveloper/)

</div>

---

<br>

<div align="center">

### 💫 Made with ❤️ by [beydeveloper](https://github.com/beydeveloper)

</div>
